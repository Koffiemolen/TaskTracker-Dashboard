...
class DataAggregationService:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, source_db_url: str, target_db_url: str):
        ...
        self.source_session_local = sessionmaker(autocommit=False, autoflush=False, bind=self.source_engine)  # pylint: disable=line-too-long
        self.target_session_local = sessionmaker(autocommit=False, autoflush=False, bind=self.target_engine)  # pylint: disable=line-too-long
    ...
    async def transfer_data(self):
        ...
        transfer_sql = text("""---https://www.sqlshack.com/understanding-the-sql-merge-statement/
...
USING (
    SELECT 
        ...
        ac.SuccessCount,
        ac.FailureCount,
        (SELECT COUNT(*) FROM [Automate11].[dbo].[workflowitemconstructs] wic WHERE wic.WorkflowID = ac.ResourceID) AS NumberOfTasks
...
WHEN NOT MATCHED BY TARGET THEN
    INSERT (ResourceID, WorkflowName, ParentID, ResourceType, CompletionState, Notes, CreatedBy, CreatedOn, ModifiedOn, LastModifiedOn, Version, VersionDate, Empty, Enabled, Removed, ResultCode, ResultText, StartedOn, EndedOn, LockedBy, SuccessCount, FailureCount, NumberOfTasks, UpdatedOn)
    VALUES (source.ResourceID, source.ResourceName, source.ParentID, source.ResourceType, source.CompletionState, source.Notes, source.CreatedBy, source.CreatedOn, source.ModifiedOn, source.LastModifiedOn, source.Version, source.VersionDate, source.Empty, source.Enabled, source.Removed, source.ResultCode, source.ResultText, source.StartedOn, source.EndedOn, source.LockedBy, source.SuccessCount, source.FailureCount, source.NumberOfTasks, GETDATE())
...
""")
    ...
    def _sync_transfer_data(self, sql_statement):
        # Execute the SQL statement in a synchronous session
